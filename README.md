# sapper-template using rollup + typescript + jest + sveltestrap

Original code was taken from [https://github.com/sveltejs/sapper-template](https://github.com/sveltejs/sapper-template)
...
To create a new Sapper project run

```bash
npx degit "ladeiko/sapper-template" my-app
```
...
Go to either [sapper-template](https://github.com/ladeiko/sapper-template) and click on "Use this template" to create a new project repository initialized by the template.
...
The default [service-worker.ts](src/service-worker.ts) will preload and cache these files, by retrieving a list of `files` from the generated manifest:
